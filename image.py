...
class Image:
    def __init__(self, x_pixels=0, y_pixels=0, num_channels=0, filename=''):
        ...
        #paths for inputs and the outputs
        self.input_path = 'input/'
        self.output_path = 'output/'
        if x_pixels and y_pixels and num_channels:
            self.x_pixels = x_pixels
            self.y_pixels = y_pixels
            self.num_channels = num_channels #num_channels - usually for colors we have red, green and blue; so that's what we work with
            self.array = np.zeros((x_pixels, y_pixels, num_channels)) #3D array
        elif filename:
    ...
    def read_image(self, filename, gamma=2.2): #pass in filename, gamma is for en and de coding
        '''
    ...
    def write_image(self, output_file_name, gamma=2.2): #write whatever the image represents to a png file
        '''
        3D numpy array (Y, X, channel) of values between 0 and 1 -> write to png
        '''
        im = np.clip(self.array, 0, 1) #because when we transform it back it's from 0 - 255
        ...
        self.array.resize(y, x, 3)  # we mutated the method in the first step of the function, because of reshaping it and all
        

if __name__ == '__main__': #checking if import/export works
